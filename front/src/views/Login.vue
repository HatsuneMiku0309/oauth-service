      ...
      <form class="mx-16 mt-6 p-4" @submit.prevent="checkForm" autocomplete="nope">
        <div class="flex flex-col items-center px-12 pb-12">
          <div class="flex flex-col my-2 items-center w-11/12">
            ...
            <input type="text" class="w-full h-10 border rounded-xl border-gray-700 px-2 bg-transparent focus:shadow-md focus:outline-none" @keyup.exact.enter="submit" v-model="account" required placeholder="Account:" />
          </div>
          <div class="flex flex-col my-2 items-center w-11/12">
            <div class="flex flex-row w-full mb-1">
              <span class="w-full text-left">Password</span>
              <span @click="testClickScope" class="w-full text-right text-blue-500 underline">forget password</span>
            </div>
            <div class="w-full relative">
              <input type="password" class="w-full h-10 border rounded-xl border-gray-700 px-2 bg-transparent focus:shadow-md focus:outline-none" @keyup.exact.enter="submit" v-model="password" required placeholder="Password:" />
            </div>
          </div>
        </div>
